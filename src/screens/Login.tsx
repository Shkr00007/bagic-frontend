...
export function Login({ onLoginSuccess }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const generatedEmployeeId = isEmailInput
        ? `EMP-${Date.now().toString().slice(-5)}`
        : inputValue.toUpperCase();

      const generatedEmail = isEmailInput
        ? inputValue.toLowerCase()
        : `${inputValue.toLowerCase()}@synthetic.local`;

      const generatedFullName = isEmailInput
        ? inputValue.split('@')[0]
        : `Employee ${generatedEmployeeId}`;

      // Store synthetic profile in session storage
      sessionStorage.setItem(
        'employeeProfile',
        JSON.stringify({
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-8 bg-[#F8FAFC]">
      <div className="w-full max-w-[480px]">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-[#E2E8F0] p-8">
          <div className="text-center mb-6">
            <h2 className="text-h2 mb-3">Synthetic Coaching Login</h2>
            <p className="text-body text-[#64748B]">
              Demo Mode – No backend – Instant access
            </p>
          </div>
