...
export function ExecutiveDashboard({ onNavigate }: ExecutiveDashboardProps) {
  ...
  const [personaMetrics, setPersonaMetrics] = useState<PersonaMetric[]>([]);
  ...
  useEffect(() => {
    ...
    // Synthetic OKRs
    setKeyResults([
      { id: '1', kr_code: 'KR1', current_value: 100, unit: '%', status: 'Met' },
      { id: '2', kr_code: 'KR2', current_value: 0, unit: '', status: 'Met' },
      { id: '3', kr_code: 'KR3', current_value: 95, unit: '%', status: 'Met' },
      { id: '4', kr_code: 'KR4', current_value: 88, unit: '%', status: 'Met' },
      { id: '5', kr_code: 'KR5', current_value: 92, unit: '%', status: 'Met' },
    ]);

    // Synthetic persona metrics
    setPersonaMetrics([
      {
        id: '1',
        persona_type: 'Branch Officer',
        total_users: 12,
        total_sessions: 48,
  ...
  const handleExportReport = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `poc-report-${new Date()
      .toISOString()
      .split('T')[0]}.json`;
    link.click();
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pl-64">
      ...
      <main className="p-8">
        <div className="max-w-[1600px] mx-auto">
          <h1 className="text-h1 mb-8">AI Coach POC - Executive Dashboard</h1>
            ...
            <>
              ...
              <div className="grid grid-cols-5 gap-4 mb-10">
                {keyResults.map((kr) => (
                  ...
                  >
                    <div className="text-caption text-[#64748B] mb-2">
                      {kr.kr_code}
                    </div>
                    <div className="text-4xl font-bold text-[#1E293B] mb-2">
                      {kr.current_value}
              ...
              <div className="grid grid-cols-2 gap-6 mb-10">
                {personaMetrics.map((persona) => (
                  ...
                  >
                    ...
                    <p>Users: {persona.total_users}</p>
                    <p>Sessions: {persona.total_sessions}</p>
                    <p>Effectiveness: {persona.effectiveness_score}%</p>
                  </div>
                ))}
              </div>
              ...
              {healthMetrics && (
                <div className="bg-white rounded-lg shadow-sm border p-6 mb-10">
                  <p>Compliance Rate: {healthMetrics.compliance_rate}%</p>
                  <p>Escalation Rate: {healthMetrics.escalation_rate}%</p>
                  <p>Active Sessions: {healthMetrics.active_sessions}</p>
                  <p>Violations Today: {healthMetrics.violations_today}</p>
                </div>
              )}
