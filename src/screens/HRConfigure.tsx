...
export function HRConfigure({ onNavigate }: HRConfigureProps) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('hr-config');

    if (stored) {
      const parsed = JSON.parse(stored);
      setFrameworkMappings(parsed.frameworkMappings);
      setBoundaryRules(parsed.boundaryRules);
      setGuardrailRules(parsed.guardrailRules);
    } else {
      // Default synthetic config
      const defaultFrameworks: FrameworkMapping[] = [
        { persona_type: 'OH-EC-IC', framework_type: 'GROW', is_enabled: true },
        { persona_type: 'OH-MC-PM', framework_type: 'CLEAR', is_enabled: true },
        { persona_type: 'OH-SL', framework_type: 'ITC', is_enabled: true },
      ];
      ...
      const defaultBoundaries: BoundaryRule[] = [
        {
          id: '1',
          rule_code: 'B1',
          rule_name: 'Confidence Drop',
          description: 'Flag significant confidence drop',
          threshold_value: 80,
      ...
      const defaultGuardrails: GuardrailRule[] = [
        {
          id: '1',
          rule_name: 'Escalation Alert',
          threshold_value: 85,
          is_enabled: true,
        },
      ];

      setFrameworkMappings(defaultFrameworks);
      setBoundaryRules(defaultBoundaries);
      setGuardrailRules(defaultGuardrails);
    }
  ...
  const handleFrameworkToggle = (
    ...
    setFrameworkMappings((prev) =>
      prev.map((m) =>
        m.persona_type === personaType &&
        m.framework_type === frameworkType
          ? { ...m, is_enabled: enabled }
          : m
      )
    );
  ...
  const handleGuardrailToggle = (id: string, enabled: boolean) => {
    setGuardrailRules((prev) =>
      prev.map((g) =>
        g.id === id ? { ...g, is_enabled: enabled } : g
      )
    );
  };
  ...
  const handleSave = () => {
    ...
    localStorage.setItem('hr-config', JSON.stringify(config));

    setSaveMessage('Configuration saved successfully');
    setTimeout(() => setSaveMessage(''), 3000);
  };

  const frameworks = ['GROW', 'CLEAR', 'ITC'];
  const personas = ['OH-EC-IC', 'OH-MC-PM', 'OH-SL'];
        ...
        <div className="max-w-[1200px] mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-body text-[#64748B]">
                Synthetic local configuration mode
              </p>
            </div>
          ...
          {saveMessage && (
            <div className="mb-6 p-4 rounded-lg bg-[#10B981]/10 border border-[#10B981]/20 text-[#10B981]">
              {saveMessage}
            </div>
          )}
          ...
          {loading ? (
            ...
            <>
              <div className="mb-6 flex gap-4">
                {['frameworks', 'boundaries', 'guardrails'].map(
                  (tab) => (
                    <button
                      key={tab}
              ...
              {activeTab === 'frameworks' && (
                <div className="space-y-4">
                  {frameworkMappings.map((mapping) => (
                    <div
                      key={`${mapping.persona_type}-${mapping.framework_type}`}
                      className="flex justify-between p-4 border rounded"
                    >
                      <span>
                        {getPersonaLabel(mapping.persona_type)} -{' '}
                        {mapping.framework_type}
                      </span>
